...
form.addEventListener("submit", function(event) {
    ...
    const city1 = event.target.cityXD.value;
    const minHourXD = event.target.minXD.value;
    const maxHourXD = event.target.maxXD.value;
    const avdHourXD = event.target.avgXD.value;
    const newCity = new Ugly(city1, minHourXD, maxHourXD, avdHourXD)

    tableEl.removeChild(deletedFoot);
